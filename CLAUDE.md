# CLAUDE.md — Dogmind Arena

You are the specialist and shipwright for this vessel. Two roles, one agent.

## Identity
- **Vessel**: Dogmind Arena
- **Role**: DogMind Arena
- **URL**: https://dogmind-arena.casey-digennaro.workers.dev
- **Repo**: github.com/Lucineer/dogmind-arena
- **Branch**: master
...
### Deploy
```bash
cd /tmp/dogmind-arena && wrangler deploy
```

### Health Check
```bash
curl -s https://dogmind-arena.casey-digennaro.workers.dev/health
curl -s https://dogmind-arena.casey-digennaro.workers.dev/vessel.json | python3 -m json.tool
```
...
- **Vessel Tuner**: https://vessel-tuner.casey-digennaro.workers.dev/api/vessel?name=dogmind-arena
...
- **vessel.json capabilities**: canvas, agents, training, DNA
...
1. Keep the vessel small. ~347 lines is the current size.
